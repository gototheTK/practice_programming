...
### Stack
Stack은 Stack Frame 단위로 나뉘어져 있으며, Thread의 새로운 메서드가 실행되거나 탈출할 때마다 Stack Frame이 생성 및 소멸을 합니다. Stack Frame에는 메서드 내부의 로컬 변수를 관리하는 Local Variables Array와 연산 과정을 일시적으로 저장하는 Operand Stack, 그리고 메서드 종료 시 돌아와야 할 주소나 그 외 메서드에 대한 정보를 가지고 있는 Frame Data로 나뉘어 있습니다.
...
Thread는 Heap, Method Area와 같은 공간을 공유하지만, Thread로 메소드를 실행할때, Local Variable Array에 있는 지역변수는 공유할 수 없습니다.


아래 예시와 같이 Coffree라는 똑같은 객체가 각각 다른 메소드에 들어갔지만, 메서드를 실행했을때 각각 다른 실행이므로 지역변수를 공유하지는 않습니다. 물론 메서드안에서 인스턴트 변수나 정적 변수를 공유하는것이 가능합니다. 그러나 특병할 경우를 제외하면은 공유되는 변수값은 수정하지 않는것이 바람직합니다.
